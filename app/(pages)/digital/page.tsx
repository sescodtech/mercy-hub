...
import {
  ...
  CheckCircle, XCircle, Loader2, RefreshCw, ArrowLeft, ShieldCheck,
} from "lucide-react";
            ...
            <div className="grid grid-cols-2 gap-4">
              {CATEGORIES.map(({ id, label, icon: Icon, color, desc }) => (
                <button
                  ...
                  className="bg-white rounded-2xl p-5 text-left border border-neutral-100 hover:shadow-md transition-all group"
                  ...
                  <ChevronRight className="w-4 h-4 text-neutral-300 mt-2" />
                </button>
              ))}
            </div>

            <div className="mt-6 bg-white rounded-2xl border border-neutral-100 p-4 flex items-center justify-between">
              <p className="text-sm font-medium text-neutral-700">Recent Orders</p>
              <Link href="/dashboard/digital-orders" className="text-xs flex items-center gap-1 hover:underline" style={{ color: "#d98c2a" }}>
                    ...
                    <>
                      ...
                      {planError && !planLoad && (
                        <div className="text-center py-6">
                          ...
                          <button
                            onClick={() => fetchPlans("data", network)}
                ...
                <>
                  ...
                  <div className="grid grid-cols-4 gap-2 mb-5">
                    {NETWORKS.map(n => (
                      <button
                        key={n.id}
                        onClick={() => { setNetwork(n.id as Network); fetchPlans("airtime"); }}
                        className="py-3 rounded-xl text-sm font-semibold border-2 transition-all"
                        style={{
                          borderColor: network === n.id ? "#d98c2a" : "#e5e5e5",
                          backgroundColor: network === n.id ? "rgba(217,140,42,0.1)" : "transparent",
        ...
        {step === "result" && result && (
          <div className="bg-white rounded-2xl border border-neutral-100 p-8 text-center">
            ...
            <div className="flex gap-3 justify-center flex-wrap">
              <button
                onClick={() => { setStep("category"); setCategory(null); setNetwork(""); setPlans([]); setPlan(null); setResult(null); setPhone(""); setSmartcard(""); }}
