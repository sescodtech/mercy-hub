    ...
    <div className="min-h-screen bg-cream">
      ...
      <div className="bg-[#1a1108] text-white py-20">
        <div className="container-site text-center">
          <p className="text-[var(--color-brand-primary)] text-xs tracking-[0.3em] uppercase font-body mb-4">Our Journal</p>
      ...
      <div className="container-site py-12">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-10">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search posts…"
              className="w-full pl-10 pr-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white outline-none focus:border-[var(--color-brand-primary)]" />
          </div>
          <div className="flex gap-2 flex-wrap">
            {CATEGORIES.map((cat) => (
              <button key={cat} onClick={() => { setCategory(cat); setPage(1); }}
                className={cn("px-4 py-2 rounded-xl text-xs font-medium transition-colors",
                  category === cat ? "bg-[var(--color-brand-primary)] text-white" : "bg-white border border-neutral-200 text-neutral-600 hover:border-[var(--color-brand-primary)]")}>
                {cat}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            {search && <button onClick={() => setSearch("")} className="mt-3 text-[var(--color-brand-primary)] text-sm underline">Clear search</button>}
          </div>
        ) : (
          <>
              ...
              {filtered.map((post) => (
                <Link key={post._id} href={`/blog/${post.slug}`} className="group bg-white rounded-2xl overflow-hidden border border-neutral-100 hover:shadow-lg transition-shadow">
                  <div className="relative h-48 bg-neutral-100">
                    ...
                    <span className="absolute top-3 left-3 px-2 py-1 bg-[var(--color-brand-primary)] text-white text-[10px] font-semibold rounded-md uppercase tracking-wide">{post.category}</span>
                  </div>
                  <div className="p-5">
                    <h2 className="font-display text-lg font-semibold text-neutral-900 mb-2 line-clamp-2 group-hover:text-[var(--color-brand-primary)] transition-colors">{post.title}</h2>
                    ...
                    <div className="flex items-center justify-between text-xs text-neutral-400">
                      ...
                      <span className="flex items-center gap-1 text-[var(--color-brand-primary)] font-medium">Read <ChevronRight className="w-3 h-3" /></span>
                    </div>
                  </div>
                </Link>
              ))}
                ...
                {Array.from({ length: pages }, (_, i) => i + 1).map((p) => (
                  <button key={p} onClick={() => setPage(p)}
                    className={cn("w-10 h-10 rounded-xl text-sm font-medium transition-colors",
                      p === page ? "bg-[var(--color-brand-primary)] text-white" : "bg-white border border-neutral-200 text-neutral-600 hover:border-[var(--color-brand-primary)]")}>
                    {p}
                  </button>
                ))}
              </div>
