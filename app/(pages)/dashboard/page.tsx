      ...
      <div className="bg-white border-b border-neutral-100">
        <div className="container-site py-6">
          <div className="flex items-center justify-between">
            ...
            <Link
              href="/shop"
              className="hidden sm:flex items-center gap-2 px-4 py-2.5 bg-[var(--color-brand-primary)] text-white text-sm font-medium rounded-xl hover:bg-[var(--color-brand-accent)] transition-colors"
            >
      ...
      <div className="container-site py-8 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {QUICK_LINKS.map(({ label, href, Icon, desc }) => (
              <Link
                key={href}
                href={href}
                className="bg-white rounded-2xl border border-neutral-100 p-5 flex items-center gap-4 hover:border-[var(--color-brand-primary-40)] hover:shadow-sm transition-all group"
              >
                <div className="w-11 h-11 bg-[var(--color-brand-primary-10)] rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-[var(--color-brand-primary)] transition-colors">
                  <Icon className="w-5 h-5 text-[var(--color-brand-primary)] group-hover:text-white transition-colors" />
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-neutral-900">{label}</p>
                  <p className="text-xs text-neutral-400 mt-0.5">{desc}</p>
                </div>
                <ArrowRight className="w-4 h-4 text-neutral-300 ml-auto flex-shrink-0 group-hover:text-[var(--color-brand-primary)] transition-colors" />
              </Link>
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/orders" className="text-sm text-[var(--color-brand-primary)] hover:underline flex items-center gap-1">
            ...
            <div className="space-y-3">
              {recentOrders.map((order: any) => (
                <Link
                  ...
                  className="bg-white rounded-2xl border border-neutral-100 p-4 sm:p-5 flex items-center gap-4 hover:border-[var(--color-brand-primary-30)] hover:shadow-sm transition-all"
                >
                  {/* Order icon */}
        ...
        <div className="sm:hidden">
          <Link
            href="/shop"
            className="flex items-center justify-center gap-2 w-full py-3.5 bg-[var(--color-brand-primary)] text-white text-sm font-semibold rounded-xl hover:bg-[var(--color-brand-accent)] transition-colors"
          >
            Continue Shopping <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
