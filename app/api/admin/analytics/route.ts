...
export async function GET(req: NextRequest) {
  try {
    ...
    const [
      ...
      Product.countDocuments({ trackInventory: true, isActive: true, $expr: { $lte: ["$stock", "$lowStockThreshold"] } }),
      Order.aggregate([
    ...
    return NextResponse.json({
      ...
      data: {
        overview: {
          ...
          avgOrderValue: revenueData[0]?.count > 0 ? Math.round(totalRevenue / revenueData[0].count) : 0,
        },
        revenueOverTime,
        topProducts,
        categoryPerformance,
