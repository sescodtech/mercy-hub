...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!shipping?.enabled) {
      return NextResponse.json({ success: true, data: { cost: 0, label: "Free Delivery", estimatedDays: "3-5", isFree: true } });
    }

    // Free shipping threshold check
    const threshold = shipping.freeShippingThreshold ?? 50000;
    if (shipping.freeShippingEnabled && orderTotal >= threshold) {
      return NextResponse.json({ success: true, data: { cost: 0, label: "Free Shipping", estimatedDays: "3-5", isFree: true } });
    }
    ...
    let cost = defaultCost, label = "Standard Delivery", estimatedDays = "5-7";

    if (lagosAreas.some((s) => stateLower.includes(s))) {
      cost = Math.round(defaultCost * 0.6); label = "Lagos Delivery"; estimatedDays = "1-2";
    } else if (southwestStates.some((s) => stateLower.includes(s))) {
      cost = Math.round(defaultCost * 0.8); label = "Southwest Delivery"; estimatedDays = "2-4";
    } else if ([...southsouthStates, ...southeastStates].some((s) => stateLower.includes(s))) {
      cost = defaultCost; label = "South Delivery"; estimatedDays = "3-5";
    } else if (northStates.some((s) => stateLower.includes(s))) {
      cost = Math.round(defaultCost * 1.4); label = "North Delivery"; estimatedDays = "5-7";
    }

    return NextResponse.json({ success: true, data: { cost, label, estimatedDays, isFree: false } });
  } catch (error) {
