        ...
        <div className="container-site">
          <div className="flex h-16 items-center justify-between gap-4">
            ...
            <Link href="/" className="flex-shrink-0 group">
              ...
              <span className="block text-[9px] tracking-[0.3em] uppercase text-neutral-400 font-body -mt-1">
                Essentials
              </span>
            </Link>
            ...
            <div className="flex items-center gap-1">
              ...
              <Link href="/dashboard/wishlist" className="btn-icon relative" aria-label="Wishlist">
                ...
                {wishlistCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-brand-500 text-white text-[10px] flex items-center justify-center font-medium">
              ...
              {session ? (
                <div className="relative group hidden lg:block">
                  <button className="btn-icon flex items-center gap-1">
                  ...
                  <div className="absolute right-0 top-full mt-1 w-48 rounded-lg bg-white border border-neutral-100 shadow-luxury overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                    ...
                    <Link href="/dashboard" className="flex items-center gap-2 px-4 py-2.5 text-sm text-neutral-700 hover:bg-neutral-50">
                      <LayoutDashboard className="w-4 h-4" /> Dashboard
                    </Link>
                    <Link href="/dashboard/orders" className="flex items-center gap-2 px-4 py-2.5 text-sm text-neutral-700 hover:bg-neutral-50">
                      <Package className="w-4 h-4" /> Orders
                    </Link>
                    {session.user?.role === "admin" && (
                      <Link href="/admin" className="flex items-center gap-2 px-4 py-2.5 text-sm text-brand-600 hover:bg-brand-50">
                ...
                <Link href="/auth/login" className="hidden lg:flex btn-primary py-2 px-4 text-xs">
                  Sign In
                </Link>
              )}
              ...
              <div className="container-site py-4 space-y-1">
                {navLinks.map((link) => (
                  <div key={link.href}>
                    <Link
                      href={link.href}
                      className="block px-3 py-2.5 text-sm font-medium text-neutral-700 rounded-md hover:bg-neutral-50"
                    >
                      {link.label}
                    </Link>
                    {link.children && (
                      <div className="ml-4 mt-1 space-y-1">
                        {link.children.map((child) => (
                          <Link
                            key={child.href}
                            href={child.href}
                            className="block px-3 py-2 text-sm text-neutral-500 rounded-md hover:bg-neutral-50"
                          >
                            {child.label}
                          </Link>
                        ))}
                ...
                <div className="pt-4 border-t border-neutral-100">
                  {session ? (
                    <>
                      ...
                      <button
                        onClick={() => signOut()}
                        className="block w-full text-left px-3 py-2.5 text-sm text-red-600"
                      >
                        Sign Out
                      </button>
                    </>
