...
import {
  ...
  Wifi, Phone, Tv, GraduationCap, Zap,
} from "lucide-react";
...
const NAV_LINKS = [
  ...
  },
  {
    ...
    href:  "/digital",
    // highlight — shown with a badge to draw attention
    badge: "New",
    children: [
      { label: "Data Bundles",       href: "/digital?category=data",      Icon: Wifi,           color: "#d98c2a" },
      { label: "Airtime Recharge",   href: "/digital?category=airtime",   Icon: Phone,          color: "#10b981" },
      { label: "Cable TV",           href: "/digital?category=cable",     Icon: Tv,             color: "#6366f1" },
      { label: "Exam PINs",          href: "/digital?category=education", Icon: GraduationCap,  color: "#f59e0b" },
      { label: "View All Services",  href: "/digital",                    Icon: Zap,            color: "#d98c2a" },
    ],
  },
  { label: "New Arrivals", href: "/shop?filter=new" },
  ...
  return (
    <>
      ...
      <header
        ...
        style={!scrolled ? { backgroundColor: "var(--color-header-bg, #fdf8f0)" } : undefined}
      >
          ...
          <div className="flex h-14 items-center justify-between gap-4">
            ...
            <nav className="hidden lg:flex items-center gap-0">
              {NAV_LINKS.map((link) => {
                ...
                const isDigital = link.href === "/digital";
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {/* Digital badge */}
                      {link.badge && (
                        <span className="ml-1 px-1.5 py-0.5 text-[9px] font-bold rounded-full bg-[#d98c2a] text-white leading-none">
                          {link.badge}
                        </span>
                      )}
                    ...
                    <AnimatePresence>
                      {link.children && activeDropdown === link.label && (
                        <motion.div
                          ...
                          className={cn(
                            "absolute top-full mt-1 rounded-xl bg-white border border-neutral-100 shadow-lg overflow-hidden",
                            isDigital ? "left-0 w-64" : "left-0 w-52"
                          )}
                        >
                          {/* Digital dropdown — with icons */}
                          {isDigital ? (
                            <div className="p-2">
                              <p className="px-3 pt-1 pb-2 text-[10px] font-bold text-neutral-400 uppercase tracking-wider">
                                Services
                              </p>
                              {link.children.map((child) => {
                                const { Icon, color, label, href } = child as {
                                  Icon: React.ElementType; color: string; label: string; href: string;
                                };
                                const isLast = label === "View All Services";
                                return (
                                  <Link
                                    key={href}
                                    href={href}
                                    className={cn(
                                      ...
                                      isLast
                                        ? "mt-1 border-t border-neutral-100 text-[#d98c2a] font-semibold hover:bg-[#fdf3e7]"
                            ...
                            link.children.map((child) => (
                              <Link
                                key={child.href}
                                href={child.href}
                                className="flex items-center px-4 py-2.5 text-sm text-neutral-600 transition-colors hover:bg-neutral-50"
                                style={{}}
                                onMouseEnter={(e) => {
              ...
              {session ? (
                <div className="relative group hidden lg:block ml-1">
                  ...
                  <div className="absolute right-0 top-full mt-2 w-52 rounded-xl bg-white border border-neutral-100 shadow-lg overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150">
                    ...
                    <Link href="/digital" className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-neutral-600 hover:bg-neutral-50 hover:text-neutral-900 transition-colors">
                      <Zap className="w-4 h-4" /> Digital Services
                    </Link>
                    {(session.user as any)?.role === "admin" && (
                      <Link
                        href="/admin"
                        className="flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors border-t border-neutral-100"
                      ...
                      >
                        {NAV_LINKS[0].children!.map((child) => (
                          <Link
                            ...
                            onClick={() => setMobileOpen(false)}
                            className="block px-2 py-2 text-sm text-neutral-500 rounded-md hover:text-[#d98c2a] transition-colors"
                          >
                            {child.label}
                          </Link>
                        ))}
                ...
                <div>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Zap className="w-4 h-4" style={{ color: "#d98c2a" }} />
                      Digital Services
                      <span className="px-1.5 py-0.5 text-[9px] font-bold rounded-full bg-[#d98c2a] text-white leading-none">
                        New
                      </span>
                    </div>
                    <ChevronDown className={cn("w-4 h-4 transition-transform text-neutral-400", mobileDigitalOpen && "rotate-180")} />
                  </button>
                  <AnimatePresence>
                    {mobileDigitalOpen && (
                      <motion.div
                        ...
                        className="overflow-hidden ml-3 border-l border-neutral-100 pl-3 mt-1"
                      >
                        {NAV_LINKS[1].children!.map((child) => {
                          const { Icon, color, label, href } = child as {
                            Icon: React.ElementType; color: string; label: string; href: string;
                          };
                          return (
                            <Link
                              key={href} href={href}
                              onClick={() => setMobileOpen(false)}
                              className="flex items-center gap-2 px-2 py-2 text-sm text-neutral-600 rounded-md hover:text-[#d98c2a] transition-colors"
                            >
                ...
                {session ? (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <Link href="/auth/register" onClick={() => setMobileOpen(false)}
                      ...
                      style={{ backgroundColor: "var(--color-button-primary, #d98c2a)", color: "var(--color-button-text, #ffffff)" }}>
                      Register
                    </Link>
                  </div>
                )}
